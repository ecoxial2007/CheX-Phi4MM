    ...
    for i in tqdm(range(N), disable=(rank != 0) or disable_tqdm, desc="Evaluation progress"):
        ...
        generated_outputs = model.generate(
            **inputs,
            ...
            max_new_tokens=256,
            do_sample=False,
            output_scores=True,  # <-- 添加
            return_dict_in_generate=True  # <-- 添加
        )
        ...
        resp = processor.batch_decode(
            generated_outputs.sequences[:, in_len:], skip_special_tokens=False, clean_up_tokenization_spaces=False
        )[0].strip()

        # --- BUG修复：提取每个token和token的prob ---
        token_prob_list = []
        token_list = []

        # generated_outputs.scores 是一个元组，包含了每一步的logits
        # 它的长度等于生成的token数量
        for step, scores in enumerate(generated_outputs.scores):
            # scores shape: [batch_size, vocab_size], batch_size在这里是1

            # 1. 计算概率
            probs = torch.softmax(scores, dim=-1)  # shape: [1, vocab_size]

            # 2. 获取这一步生成的token ID
            # generated_outputs.sequences shape: [1, in_len + generated_tokens]
            # 我们需要索引 [0, in_len + step]
            generated_token_id = generated_outputs.sequences[0, in_len + step]

            # 3. 获取该token的概率
            token_prob = probs[0, generated_token_id].item()

            # 4. 解码该token
            token = processor.decode([generated_token_id], skip_special_tokens=True, clean_up_tokenization_spaces=False)
        ...
        for token, prob in zip(token_list, token_prob_list):
        ...
        all_results.append({
            ...
            "answer": gts,
            "explanation": exp,
            "generated_text": resp,  # 完整的句子
            "generated_tokens": token_list,  # 每个token
            "token_probabilities": token_prob_list,  # 每个token的prob
            "is_correct": ok,
...
def main():
    ...
    parser.add_argument('--base_model_path', type=str, default='../02_Phi4-MM/base_model/Phi-4-multimodal-instruct',
                       help='Base model path for image processor')
    parser.add_argument('--annotation_file', type=str, default=None, help='Optional: single test json file')
    parser.add_argument('--img_root', type=str, default='./data/images/MIMIC_CXR_JPG',
                        help='Optional: image root directory for annotation_file')
    ...
    processor, processor_source = load_processor_no_special_tokens(
        # base_model_path=args.base_model_path, # 你的代码注释了这行
    ...
    if args.annotation_file:
        ...
        eval_dataset = create_dataset_multi(processor, folder=args.dataset_folder)
