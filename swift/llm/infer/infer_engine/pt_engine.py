...
class PtEngine(InferEngine):
    ...
    #                 per_token = []
    #                 for step_token_id, step_dict in zip(generate_ids, logprobs_list):
    #                     # step_dict: {token_id: logprob, ...}
    #                     lp = step_dict.get(step_token_id)
    #                     if lp is None:
    #                         continue
    #                     per_token.append({
    #                         'token_id': int(step_token_id),
    #                         'token': self.tokenizer.convert_ids_to_tokens([int(step_token_id)])[0],
    #                         'logprob': float(lp),
    #                         'prob': float(math.exp(lp))
    #                     })
    #                 if logprobs is None:
    #                     logprobs = {}
    #                 logprobs['per_token'] = per_token
    ...
    def _infer_full(self,
        ...
        batched_generate_ids = output['sequences']
        batched_generate_ids = template.get_generate_ids(batched_generate_ids, num_prompt_tokens)
        template.debug_logger({'generate_ids': batched_generate_ids})
        batched_logprobs = self.preprocess_logits(
            output.get('logits'), batched_generate_ids, generation_config.top_logprobs)
        ...
        for i in range(inputs['attention_mask'].shape[0]):
            ...
            for j in range(num_return_sequences):
                ...
                logprobs_list = None
                if batched_logprobs is not None:
                    logprobs_list = [lp for m, lp in zip(masks, batched_logprobs[batched_index]) if m.item()]

                # 覆盖默认的 logprobs
                per_token = []
                if logprobs_list is not None:
                    for step_token_id, step_dict in zip(generate_ids, logprobs_list):
                        lp = step_dict.get(int(step_token_id))
                        if lp is None:
                            continue
                        prob = math.exp(lp)
                        raw_tok = self.tokenizer.convert_ids_to_tokens([int(step_token_id)])[0]
                        pretty_tok = self.tokenizer.convert_tokens_to_string([raw_tok])
                        # per_token.append({
                        #     'token_id': int(step_token_id),
                        #     'token': self.tokenizer.convert_ids_to_tokens([int(step_token_id)])[0],
                        #     'token_prob': float(prob),
                        #     'token_logprob': float(lp)
                        # })
                        per_token.append({
                            'token_id': int(step_token_id),
                            'token_raw': raw_tok,           
                ...
                logprobs = {'per_token': per_token} if per_token else None
