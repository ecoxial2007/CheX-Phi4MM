...
class SwiftInfer(SwiftPipeline):
    ...
    def infer_dataset(self) -> List[Dict[str, Any]]:
        ...
        request_config.top_logprobs = 0  # 只保留生成 token 自身概率
        logger.info(f'request_config: {request_config}')
        ...
        if request_config and request_config.stream:
            ...
            for data, resp, labels in zip(val_dataset, resp_list, labels_list):
                ...
                token_probs, token_preds = None, None
                if logprobs_obj and 'per_token' in logprobs_obj:
                    per_token = logprobs_obj['per_token']
                    token_probs = [x['token_prob'] for x in per_token]
                    # token_preds = [x['token'] for x in per_token]
                    token_preds = [x.get('printable_token', x['token']) for x in per_token]
                record = {
                    'response': response,
                    'labels': labels,
                ...
                if token_probs is not None:
                    record['token_probs'] = token_probs
                    record['token_preds'] = token_preds
                record.update(data)
