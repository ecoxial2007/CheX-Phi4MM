...
class Template(ProcessorMixin):
    ...
    def generate(self, model, *args, **kwargs):
        # 确保传递的 past_key_values 是新的格式
        if 'past_key_values' in kwargs and isinstance(kwargs['past_key_values'], tuple):
            # 转换旧的 cache 格式
            kwargs['past_key_values'] = self._convert_to_dynamic_cache(kwargs['past_key_values'])
        
        return model.generate(*args, **kwargs)
    
    def _convert_to_dynamic_cache(self, legacy_cache):
