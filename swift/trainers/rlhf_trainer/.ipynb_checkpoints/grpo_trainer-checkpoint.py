...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def __init__(self,
        ...
        self._textual_logs = {
            ...
            'per_token_logps': deque(maxlen=maxlen),
            'raw_completion_tokens': deque(maxlen=maxlen),
        }
        ...
        if self.multi_turn_func:
            ...
            for i, output in enumerate(results):
                ...
                for choice in output.choices:
                    ...
                    # Extract tokenized completion for logging
                    tokenized_completion = []
                    if hasattr(choice, 'logprobs') and choice.logprobs and 'per_token' in choice.logprobs:
                        # Extract tokenized text from logprobs if available
                        for token_info in choice.logprobs['per_token']:
                            if 'token' in token_info:
                                tokenized_completion.append(token_info['token'])
                    else:
                        # Fallback: try to tokenize the completion text
                        try:
                            if hasattr(self.engine, 'tokenizer'):
                                tokenizer = self.engine.tokenizer
                            elif hasattr(self.template, 'tokenizer'):
                                tokenizer = self.template.tokenizer
                            else:
                                tokenizer = self.processing_class
                            
                            # Tokenize the completion and decode each token
                            tokens = tokenizer.encode(choice.message.content, add_special_tokens=False)
                            for token_id in tokens:
                                token_text = tokenizer.decode([token_id], skip_special_tokens=False)
                                tokenized_completion.append(token_text)
                        except Exception as e:
                            # If tokenization fails, just use the original text
                            tokenized_completion = [choice.message.content]
                    
                    # Store tokenized completion in the input for later logging
                    _input['tokenized_completion'] = tokenized_completion
                    _choices.append((_input['messages'], choice.finish_reason, {'tokenized_completion': tokenized_completion}))
    ...
    def _generate_completions(self, inputs: InputsType) -> InputsType:
        ...
        for i, output in enumerate(outputs):
            ...
            # Store tokenized completion if available
            if len(output[0]) > 2 and isinstance(output[0][2], dict) and 'tokenized_completion' in output[0][2]:
                inputs[i]['tokenized_completion'] = output[0][2]['tokenized_completion']
    ...
    def _generate_and_score_completions(self, inputs: InputsType) -> InputsType:
        ...
        # Calculate per_token_logps for logging
        per_token_logps_list = []
        for mini_batch in batch_encoded_inputs:
            with torch.no_grad():
                mini_batch_per_token_logps = self._get_per_token_logps(self.model, mini_batch)
                per_token_logps_list.append(mini_batch_per_token_logps)

        # Collect tokenized completions for logging
        tokenized_completions = []
        for inp in inputs:
            if 'tokenized_completion' in inp:
                tokenized_completions.append(inp['tokenized_completion'])
            else:
                # Fallback: create a simple tokenized version
                completion_text = inp['messages'][-1]['content']
                try:
                    if hasattr(self.template, 'tokenizer'):
                        tokenizer = self.template.tokenizer
                    else:
                        tokenizer = self.processing_class
                    tokens = tokenizer.encode(completion_text, add_special_tokens=False)
                    tokenized = [tokenizer.decode([token_id], skip_special_tokens=False) for token_id in tokens]
                    tokenized_completions.append(tokenized)
                except Exception:
                    tokenized_completions.append([completion_text])

        self._log_metrics(batch_encoded_inputs, messages, completions, total_rewards, total_rewards_per_func, per_token_logps_list, tokenized_completions)
    ...
    def _dynamic_sampling(self, inputs, rewards, rewards_per_func, completions):
        ...
        while resample_count < self.args.max_resample_times:
            ...
            valid_mask = (group_std > 0.06).repeat_interleave(self.num_generations)
            all_inputs = gather_object(inputs)
    ...
    def _log_metrics(self, inputs, messages, completions, rewards, rewards_per_func, per_token_logps=None, tokenized_completions=None):
        ...
        # Log per_token_logps if provided
        if per_token_logps is not None:
            # Convert per_token_logps to list format for logging
            per_token_logps_list = []
            for logps in per_token_logps:
                if isinstance(logps, torch.Tensor):
                    per_token_logps_list.append(logps.detach().cpu().tolist())
                else:
                    per_token_logps_list.append(logps)
            self._textual_logs['per_token_logps'].extend(gather_object(per_token_logps_list))
        
        # Log tokenized completions if provided
        if tokenized_completions is not None:
            self._textual_logs['raw_completion_tokens'].extend(gather_object(tokenized_completions))
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # print('DEBUG ref_per_token_logps',ref_per_token_logps)
        # print('DEBUG per_token_logps', per_token_logps)
        ...
        # print('DEBUG per_token_kl',per_token_kl)
        if self.beta != 0.0:
        ...
        # print("DEBUG per_token_loss:", per_token_loss)
        # Log the metrics
        metrics = {}
        mode = 'eval' if self.control.should_evaluate else 'train'
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if self.accelerator.is_main_process and self.log_completions:
            table = {
                ...
                'per_token_logps': self._textual_logs['per_token_logps'],
                'raw_completion_tokens': self._textual_logs['raw_completion_tokens'],
