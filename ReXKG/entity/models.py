...
class EntityModel():
    ...
    def move_model_to_cuda(self):
        ...
        gpu_id = 0
        #
        #logger.info('# GPUs = %d'%(torch.cuda.device_count()))logger.info(f'Moving model to GPU {gpu_id}...')
        if torch.cuda.is_available() and gpu_id < torch.cuda.device_count():
            # 设置要使用的 GPU
            torch.cuda.set_device(gpu_id)
            # 将模型移动到指定的 GPU
            self.bert_model = self.bert_model.to(f'cuda:{gpu_id}')
            self._model_device = f'cuda:{gpu_id}'
    ...
    def run_batch(self, samples_list, try_cuda=True, training=True):
        ...
        max_length = 500  # BERT 的最大长度

# 截断 spans_tensor（假设它是一个二维张量：[batch_size, span_length]）
        if tokens_tensor.size(1) > max_length:
            tokens_tensor = tokens_tensor[:, :max_length]
        if bert_spans_tensor.size(1) > max_length:
            bert_spans_tensor = bert_spans_tensor[:, :max_length]
        bert_spans_tensor = torch.clamp(bert_spans_tensor, max=max_length - 1)
        if spans_mask_tensor.size(1) > max_length:
            spans_mask_tensor =spans_mask_tensor[:, :max_length]
        if spans_ner_label_tensor.size(1) > max_length:
            spans_ner_label_tensor = spans_ner_label_tensor[:, :max_length]
        if attention_mask_tensor.size(1) > max_length:
            attention_mask_tensor =attention_mask_tensor[:, :max_length]
        ...
        if training:
            ...
            for i, sample in enumerate(samples_list):
                ...
                prob = []
                lh = []
                if i < len(predicted_label):
                    for j in range(min(len(sample['spans']), 500)):
