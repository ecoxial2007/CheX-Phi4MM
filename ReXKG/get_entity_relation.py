...
def main_entity(args, data, device: Optional[torch.device] = None):
    if device is None:
        device = torch.device(
...
def main_relation(args, js, device: Optional[torch.device] = None):
    model = 'utils/test_REXKG/BiomedNLP'
    if device is None:
        device = torch.device(
            f"cuda:{torch.cuda.current_device()}"
            if torch.cuda.is_available() and not args.no_cuda2
            else "cpu"
        )
